...
use utils::{find_max_less_than, find_minecraft_text_section};
...
fn init() {
    ...
    let mcmap =
        find_minecraft_text_section().expect("Cannot find libminecraftpe.so in memory maps");
    ...
    let Some(function_addr) = find_max_less_than(&possible_fn_starts, water_mob_cap.unwrap())
    else {
